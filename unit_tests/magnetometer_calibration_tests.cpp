...
TEST(Calibration, MagnetometerCalibrator_Feed_less_than)
{
   ...
   //EXPECT_TRUE(mag_cal.get_feed_count() == 2);
   //EXPECT_TRUE(mag_cal.get_region_sample_count());
   //EXPECT_TRUE(mag_cal.get_region_sample_value());
   //EXPECT_TRUE(mag_cal.get_sphere_coverage_quality() == 1);
   //TODO
   FAIL();
}
