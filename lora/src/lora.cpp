...
LoRa<SpiDeviceClass>::LoRa(const std::string &address, const bool as_receiver, const LoRa::Frequency frequency_mhz_, const LoRa::Bandwidth bandwidth_khz_, const LoRa::SpreadFactor spread_factor_)
...
template <class SpiDeviceClass>
LoRa<SpiDeviceClass>::LoRa(const std::string &address, const bool as_receiver)
: LoRa(address, as_receiver, LoRa::Frequency::FREQ_868, LoRa::Bandwidth::BW_500K, LoRa::SpreadFactor::SF_12)
{
}

template <class SpiDeviceClass>
LoRa<SpiDeviceClass>::~LoRa()
{
...
template <class SpiDeviceClass>
void LoRa<SpiDeviceClass>::configure()
{
    ...
    write_register(OPMODE_ADDRESS, op_mode);
    write_register(FRFMSB_ADDRESS, FRFMSB);
    write_register(FRFMIB_ADDRESS, FRFMIB);
    write_register(FRFLSB_ADDRESS, FRFLSB);

    // Set bandwidth to specified option.
    uint8_t modem_conf = read_register(MODEMCONFIG1_ADDRESS);
    ...
    modem_conf |= (uint8_t)(bandwidth_khz);
    modem_conf &= ~MODEMCONFIG1_IMPLHDR;
    write_register(MODEMCONFIG1_ADDRESS, modem_conf);

    // Set spread factor to 12.
    modem_conf = read_register(MODEMCONFIG2_ADDRESS);
    ...
    modem_conf |= (uint8_t)(spread_factor);
    // Always enable CRC
    modem_conf |= MODEMCONFIG2_RXCRCON;
    write_register(MODEMCONFIG2_ADDRESS, modem_conf);
    ...
    uint8_t PA_config = read_register(PACONFIG_ADDRESS);
    PA_config |= PACONFIG_PABOOST;
    PA_config |= PACONFIG_MAXPWR(7);
    PA_config |= PACONFIG_OUTPWR(0xf);
    write_register(PACONFIG_ADDRESS, PA_config);
    ...
    write_register(LNA_ADDRESS, LNA);

    write_register(OPMODE_ADDRESS, op_mode);
    write_register(IRQFLAGS_ADDRESS, 0xff); //clear flags
}

template <class SpiDeviceClass>
void LoRa<SpiDeviceClass>::send_message(uint8_t* message, size_t num_bytes)
{
    uint8_t FIFOTxBase = read_register(FIFOTXBASE_ADDRESS); //get FIFO write position

    write_register(FIFOPTR_ADDRESS, FIFOTxBase); //set FIFO pointer to write position
    write_register(PAYLOADLEN_ADDRESS, num_bytes); //payload is this long
    write_FIFO((char*)message, num_bytes); //write data into FIFO
    write_register(IRQFLAGS_ADDRESS, 0xff); //clear flags

    // enter TX mode
    uint8_t op_mode = read_register(OPMODE_ADDRESS);
    ...
    do
    {
        IRQFlag = read_register(IRQFLAGS_ADDRESS);
    } while ((IRQFlag & IRQ_TXDONE) == 0);

    // Clear TX flag.
    write_register(IRQFLAGS_ADDRESS, IRQ_TXDONE);

    // Set the mode back to what it was before.
    write_register(OPMODE_ADDRESS, previous_op_mode);
}

/** \brief Transmits arbitrary message encoded as a string. */
template <class SpiDeviceClass>
void LoRa<SpiDeviceClass>::send_message(const std::string &message)
{
...
*/
template <class SpiDeviceClass>
uint8_t LoRa<SpiDeviceClass>::test_chip()
{
    return read_register(0x02);
}

template <class SpiDeviceClass>
uint8_t LoRa<SpiDeviceClass>::read_register(uint8_t address)
{
...
template <class SpiDeviceClass>
void LoRa<SpiDeviceClass>::write_register(uint8_t address, uint8_t value)
{
...
template <class SpiDeviceClass>
void LoRa<SpiDeviceClass>::write_FIFO(const char *str, uint8_t num_bytes)
{
...
void LoRa<SpiDeviceClass>::read_FIFO(uint8_t num_bytes, uint8_t* out_buffer)
{
    ...
    spi.write(&fifo_addr, 1);
    spi.read(out_buffer, num_bytes + 1);
}

template <class SpiDeviceClass>
bool LoRa<SpiDeviceClass>::rx(uint8_t* out_buffer)
{
    uint8_t IRQFlags = read_register(IRQFLAGS_ADDRESS);
    ...
    {
        write_register(IRQFLAGS_ADDRESS, IRQMASK_RXDONE | IRQ_PAYLOADCRCERR);
    }
 ...
 */
template <class SpiDeviceClass>
std::tuple<bool, std::string> LoRa<SpiDeviceClass>::receive_string(int timeout_ms)
{
...
template <class SpiDeviceClass>
std::tuple<bool, std::vector<uint8_t>> LoRa<SpiDeviceClass>::receive_bytes()
{
...
template class LoRa<SPIDEVICEDRIVER>;
// template class LoRa<MockSpiDevice>;
