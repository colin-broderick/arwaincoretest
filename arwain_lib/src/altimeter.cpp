...
void Altimeter::run_inference()
{
    ...
    while (mode == arwain::OperatingMode::Inference)
    {
        ...
        altitude = BMP384<PlatformI2CDevice>::calculate_altitude(pressure / 100.0, CONSTANT_ROOM_TEMPERATURE, arwain::config.sea_level_pressure);
...
{
    bmp384 = BMP384<PlatformI2CDevice>{arwain::config.pressure_address, arwain::config.pressure_bus};
    auto [p0, t0] = bmp384.read();
    p0 = p0 - arwain::config.pressure_offset;
    altitude = BMP384<PlatformI2CDevice>::calculate_altitude(p0 / 100.0, CONSTANT_ROOM_TEMPERATURE, arwain::config.sea_level_pressure);
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        altitude = BMP384<PlatformI2CDevice>::calculate_altitude(p / 100.0, CONSTANT_ROOM_TEMPERATURE, arwain::config.sea_level_pressure);
        altitude = sabatini_filter.update(arwain::rolling_average_accel_z_for_altimeter.get_value() - arwain::config.gravity, altitude);
        sleep_ms(50);
    }
    altitude_zero = altitude;
