...
{
    BMP384<PlatformI2CDevice> bmp384{arwain::config.pressure_address, arwain::config.pressure_bus};
    ...
    altitude = BMP384<PlatformI2CDevice>::calculate_altitude(pressure / 100.0, temperature, arwain::config.sea_level_pressure);
    ...
    while (!loop_timer.finished())
    {
        ...
        double new_alt = BMP384<PlatformI2CDevice>::calculate_altitude(new_pressure / 100.0, new_temperature, arwain::config.sea_level_pressure);
...
{
    // Initialize the IMU.
    IIM42652<PlatformI2CDevice> imu{i2c_address, i2c_bus};
...
{
    LIS3MDL<PlatformI2CDevice> magn{arwain::config.magn_address, arwain::config.magn_bus};
...
{
    IIM42652<PlatformI2CDevice> imu{config.imu1_address, config.imu1_bus};
    imu.set_gyro_bias(arwain::config.gyro1_bias.x, arwain::config.gyro1_bias.y, arwain::config.gyro1_bias.z);
